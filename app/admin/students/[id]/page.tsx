          ...
          <CardContent className="pt-6">
            ...
            <div className="mt-5 space-y-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>+998 90 {100 + numericId} {200 + numericId} 30</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Phone className="h-4 w-4 text-[var(--muted-foreground)] shrink-0" />
                <span className="text-[var(--muted-foreground)]">Ota-ona:</span>
            ...
            <div className="mt-5 pt-4 border-t border-[var(--border)]">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono font-bold text-[#1E3A5F] text-sm bg-[#1E3A5F]/10 px-2 py-0.5 rounded">
                    {studentId}
                  </span>
                </div>
              ...
              <Button variant="outline" size="sm" className="w-full mt-3" onClick={copyCredentials}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <Card>
              <CardContent className="pt-4 pb-4 text-center">
                <p className="text-2xl font-bold text-[#1E3A5F]">{(numericId % 3) + 1}</p>
            ...
            <Card>
              <CardContent className="pt-4 pb-4 text-center">
                ...
                <p className="text-xs text-[var(--muted-foreground)]">Qarzdorlik</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardContent className="pt-5">
                  ...
                  <TableBody>
                    {mockPayments.map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell>{getStatusLabel(p.method)}</TableCell>
                        <TableCell>
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${getStatusColor(p.status)}`}>
                  ...
                  <TableBody>
                    {mockAttendances.map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell>
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${getStatusColor(a.status)}`}>
                  ...
                  <TableBody>
                    {mockGrades.map((g) => (
                      <TableRow key={g.id}>
                        ...
                        <TableCell>{getStatusLabel(g.type)}</TableCell>
                        <TableCell className="font-medium">{g.score}/{g.maxScore}</TableCell>
                        <TableCell className="text-[var(--muted-foreground)]">{g.comment || "—"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              {activeTab === "groups" && (
                <div className="space-y-2">
                  {[
                    { id: "1", name: "Ingliz tili — A2", monthlyFee: 500000, status: "ACTIVE" as const },
                    { id: "2", name: "Matematika — B1", monthlyFee: 450000, status: "ACTIVE" as const },
                  ].slice(0, (numericId % 3) + 1).map((g) => (
                    <div key={g.id} className="flex items-center justify-between p-3 border border-[var(--border)] rounded-lg">
                      <div>
                        <p className="font-medium text-sm">{g.name}</p>
                        <p className="text-xs text-[var(--muted-foreground)]">{formatCurrency(g.monthlyFee)}/oy</p>
                      </div>
