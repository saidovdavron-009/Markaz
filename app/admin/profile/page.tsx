...
const recentActivity = [
  { action: "Yangi o'quvchi qo'shildi", detail: "Alibek Karimov — ID001", time: "2 soat oldin" },
  { action: "To'lov tasdiqlandi", detail: "ID002 — 500 000 so'm", time: "5 soat oldin" },
  { action: "Guruh yaratildi", detail: "Ingliz tili C1", time: "Kecha" },
];
...
export default function AdminProfilePage() {
  ...
  const loginStr = user?.email || "ADMIN001";
  const adminId = /^admin\d+$/i.test(loginStr) ? loginStr.toUpperCase() : "ADMIN001";

  const [info, setInfo] = React.useState({
    firstName: user?.profile?.fullName?.split(" ")[0] || "Admin",
    lastName: user?.profile?.fullName?.split(" ")[1] || "Demo",
    phone: "+998 90 123 45 67",
    dob: "01.01.1985",
    gender: "Erkak",
    role: "Administrator",
    email: user?.email || "admin@demo.uz",
    center: "EduCenter Pro",
  });
  ...
  const nonEditableFields = new Set(["role"]);

  const fieldLabels: Record<string, string> = {
    firstName: "Ism", lastName: "Familiya", phone: "Telefon raqam",
    dob: "Tug'ilgan sana", gender: "Jinsi", role: "Rol",
    email: "Email", center: "O'quv markaz",
  };

  const startEdit = (field: string) => {
    if (nonEditableFields.has(field)) return;
    setEditField(field);
  ...
  return (
    <div className="space-y-4 sm:space-y-5 max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <InfoCard title="Kirish (Login)" value={adminId} />
      ...
      {/* Recent activity */}
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 sm:p-6">
        <h2 className="text-base font-bold mb-4">So'nggi faoliyat</h2>
        <div className="space-y-3">
          {recentActivity.map((item, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-[var(--muted)]/30 border border-[var(--border)]">
              <div className="h-2 w-2 rounded-full bg-[#1E3A5F] mt-1.5 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold">{item.action}</p>
                <p className="text-xs text-[var(--muted-foreground)]">{item.detail}</p>
              </div>
              <p className="text-xs text-[var(--muted-foreground)] shrink-0">{item.time}</p>
            </div>
          ))}
        </div>
      </div>
