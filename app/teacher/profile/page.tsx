...
const myGroups = [
  { name: "Ingliz tili A2", students: 10, schedule: "Du, Ch, Ju 09:00–11:00", salary: "25 000 so'm/soat" },
  { name: "Ingliz tili B1", students: 8, schedule: "Se, Pa 11:00–13:00", salary: "25 000 so'm/soat" },
  { name: "Speaking Club", students: 6, schedule: "Sh 14:00–16:00", salary: "25 000 so'm/soat" },
];
...
export default function TeacherProfilePage() {
  ...
  const [info, setInfo] = React.useState({
    firstName: user?.profile?.fullName?.split(" ")[0] || "Sardor",
    lastName: user?.profile?.fullName?.split(" ")[1] || "Toshmatov",
    phone: "+998 90 987 65 43",
    dob: "10.05.1990",
    gender: "Erkak",
    subjects: "Ingliz tili",
    experience: "5 yil",
    email: user?.email || "teacher@demo.uz",
  });
  ...
  const fieldLabels: Record<string, string> = {
    ...
    dob: "Tug'ilgan sana", gender: "Jinsi", subjects: "Fan",
    experience: "Tajriba", email: "Email",
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-5 max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <InfoCard title="Kirish (Login)" value={info.email} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {myGroups.map((g, i) => (
            <div key={i} className="border border-[var(--border)] rounded-xl p-4 bg-[var(--muted)]/30">
              ...
              <p className="text-xs text-[var(--muted-foreground)] mt-1">{g.students} ta o'quvchi</p>
              <p className="text-xs text-[var(--muted-foreground)]">{g.schedule}</p>
              <p className="text-xs font-medium text-[#1E3A5F] mt-2">{g.salary}</p>
            </div>
          ))}
        </div>
      </div>
